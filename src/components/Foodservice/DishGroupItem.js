...
import CategoryDishList from "./MenuByGroup/CategoryDishList";

const DishGroupItem = ({ item, index }) => {
  const { removeDishGroup, fetchDishGroup } = useContext(MyServiceContext);
  ...
  return (
    <tr className="bg-white" key={item.id}>
      ...
      <td className="border border-gray-400 px-4 py-2 flex justify-center items-center gap-4">
        {/* edit */}
        <div>
          <button
            type="button"
        ...
        <button>
          <Link
            to="/staff/groupeddishes"
            onClick={() => fetchDishGroup(item)}
            class="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-amber-700x transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group"
          >
            <span class="absolute bottom-0 left-0 w-full h-1 transition-all duration-150 ease-in-out bg-[#6F4F37] group-hover:h-full"></span>
